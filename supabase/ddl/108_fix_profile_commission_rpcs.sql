-- 108_fix_profile_commission_rpcs.sql
-- แก้ไขและสร้าง RPC สำหรับดึงข้อมูลค่าคอมมิชชั่นของโปรไฟล์ตามเดือน (Fix 404)

-- ลบ functions เก่าทิ้งก่อน (ถ้ามี)
DROP FUNCTION IF EXISTS public.get_profile_monthly_commission_summary(uuid, date, date);
DROP FUNCTION IF EXISTS public.get_profile_monthly_commission_details(uuid, date, date);
DROP FUNCTION IF EXISTS public.get_profile_monthly_commission_det(uuid, date, date);
...
GRANT EXECUTE ON FUNCTION public.get_profile_monthly_commission_summary(UUID, DATE, DATE)
