-- 101_fix_payment_rpcs.sql
-- แก้ไข RPC สำหรับระบบชำระเงิน (Fix 404 / Signature mismatch)

-- 1) ลบ function เดิม (ถ้ามี) เพื่อกันสับสนเรื่อง signature
...
BEGIN
    ...
    SELECT
        d.id AS deal_id,
        d.grand_total AS total_amount,   -- ใช้ grand_total เป็นยอดที่ต้องชำระจริง
        COALESCE(SUM(
...
----------------------------------------------------------
-- 4) ให้สิทธิ์ execute RPC กับ role ต่าง ๆ
----------------------------------------------------------
