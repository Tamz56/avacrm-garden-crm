...
const ZoneDetailPage = ({ zoneId, onBack }: { zoneId: string; onBack: () => void }) => {
    ...
    // --- Tag Lifecycle Totals (สำหรับการ์ด "พร้อมขายในแปลง") ---
    const {
        data: tagLifeTotals,
    ...
    // --- Plot Totals (ดึงจาก view_plot_tree_inventory_totals เพื่อให้เบาและแม่นยำ) ---
    ...
    // --- Callback หลังสร้าง/ลบ/แก้ Tag เพื่อ refresh totals ---
    ...
    const inventorySummary = useMemo(() => {
        ...
        // totals จาก view โดยตรง (source of truth)
    ...
    if (error) {
        return (
            <div className="p-4 bg-rose-50 text-rose-600 rounded-xl border border-rose-100">
                ...
                <p>{error}</p>
                <button onClick={onBack} className="mt-2 text-sm underline">กลับไปหน้าแปลงทั้งหมด</button>
            </div>
        );
    }
    if (!zone) return null;
    ...
    return (
        <div className="flex flex-col gap-4 pb-24">
                    ...
                    <div>
                        ...
                        <div className="mt-2 flex flex-wrap items-center gap-2">
                            ...
                            <span className={"inline-flex items-center gap-1 rounded-full border px-2.5 py-1 text-xs font-medium " + mismatchClass}>
                                {mismatch?.mismatch_status === "ตรงตามระบบ" ? <CheckCircle2 className="h-3.5 w-3.5" /> : <AlertTriangle className="h-3.5 w-3.5" />}
                                {mismatchLabel}
                            </span>
                            <span className="inline-flex items-center gap-1 rounded-full border border-slate-200 bg-slate-50 px-2.5 py-1 text-xs text-slate-600">
            ...
            <section className="grid gap-3 grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-5 mb-6">
                {/* 1. จำนวนต้นไม้ทั้งหมดในแปลง (จากระบบ) - แสดง helper ชัดว่าไม่ใช่พร้อมขาย */}
                <div className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm">
                    <div className="text-xs font-medium text-slate-500">จำนวนต้นไม้ทั้งหมดในแปลง (ระบบ)</div>
                    <div className="mt-1 text-2xl font-bold text-slate-900">
                        {zoneInvLoading ? "..." : toThaiNumber(zoneInvSummary?.trees_in_plot_now ?? inventorySummary.totalPlanted)} <span className="text-sm font-normal">ต้น</span>
                    </div>
                ...
                {/* 2. พร้อมขาย (จาก Tag) — ใช้ available_qty จาก Tag lifecycle เท่านั้น */}
                <div className="rounded-2xl border border-emerald-100 bg-emerald-50/50 p-4 shadow-sm">
                    ...
                    <div className="mt-1 text-2xl font-bold text-emerald-700">
                        {tagLifeLoading ? "..." : toThaiNumber(tagLifeTotals?.available_qty ?? 0)} <span className="text-sm font-normal">ต้น</span>
                    </div>
                    <div className="mt-1 text-[11px] text-emerald-600">
                        สถานะ: in_zone (ยังไม่จอง/ยังไม่สั่งขุด)
                    </div>
                </div>
                ...
                <div className="rounded-2xl border border-amber-100 bg-amber-50/50 p-4 shadow-sm">
                    ...
                    <div className="mt-1 text-2xl font-bold text-amber-700">
                        {tagLifeLoading ? "..." : toThaiNumber(tagLifeTotals?.reserved_qty ?? 0)} <span className="text-sm font-normal">ต้น</span>
                    </div>
                    <div className="mt-1 text-[11px] text-amber-600">
                        สถานะ: reserved (จองไว้แล้ว รอดำเนินการ)
                    </div>
                </div>

                {/* 4. จำนวนต้นจากการสำรวจล่าสุด */}
                <div className="rounded-2xl border border-sky-100 bg-sky-50/50 p-4 shadow-sm">
                    ...
                    <div className="mt-1 text-2xl font-bold text-sky-700">
                        {zoneInvLoading ? "..." : toThaiNumber(zoneInvSummary?.latest_inspection_qty ?? inspectedTotal)} <span className="text-sm font-normal">ต้น</span>
                    </div>
                ...
                <div className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm">
                    ...
                    <div className="mt-1 text-lg font-bold text-slate-900">{zone.area_rai ?? zoneInvSummary?.area_rai ?? "-"} ไร่</div>
                    <div className="mt-1 text-[11px] text-slate-500">
                        {zone.area_width_m && zone.area_length_m
                            ? `ขนาดโดยประมาณ ${zone.area_width_m}×${zone.area_length_m} ม.`
                            : "รายละเอียดขนาด: -"}
                    </div>
            ...
            {activeTab === 'tags' && (
                <>
                    {/* SECTION 2: สถานะต้นไม้ในแปลงนี้ */}
            ...
            {/* === SECTION: กำหนดจำนวนต้นไม้ในแปลง (ระบบ) - Overview/Tags Tab === */}
                    ...
                    <div className="flex items-center justify-between gap-2">
                        <div>
                            ...
                            <p className="text-xs text-slate-500">
                                บันทึกลงตาราง planting_plot_trees (มีผลกับสรุประบบ/Inventory Flow)
                            </p>
                        </div>
                                        ...
                                        <tr key={d.id} className="border-b border-slate-50 hover:bg-slate-50">
                                            ...
                                            <td className="px-3 py-2 text-right">
                                                <div className="flex items-center justify-end gap-1">
                                                    {/* ปุ่มโตขึ้น/ย้ายขนาด - แสดงเฉพาะเมื่อมี species และ size */}
                                                    {d.species_id && d.size_label && Number(d.planted_count) > 0 && (
                                                        <button
                                                            ...
                                                            onClick={() => {
                                                                ...
                                                                if (hasDirty) {
                                                                    if (!window.confirm("มีข้อมูลที่แก้ไขแต่ยังไม่บันทึก ต้องการดำเนินการต่อหรือไม่?\n(ข้อมูลที่ยังไม่บันทึกจะถูก reset หลังย้ายเสร็จ)")) {
                                                                        return;
                                                                    }
                                                                }
                                                                const sp = speciesOptions.find((x) => x.id === d.species_id);
                        ...
                        <div className="space-y-3">
                            <div>
                                    ...
                                    {(() => {
                                        ...
                                        const toOptions = trunkSizeOptions
                                            ...
                                            .filter(opt => {
                                                ...
                                                return toIdx > fromIdx; // โตขึ้น = ต้องมากกว่า
                                            });
                                ...
                                {transitionReason === "growth" && (
                                    <p className="mt-1 text-[11px] text-slate-500">เมื่อเลือก "โตขึ้น" จะแสดงเฉพาะขนาดที่ใหญ่ขึ้นเท่านั้น</p>
                                )}
                            </div>
                            ...
                            <div>
                                ...
                                <input
                                    ...
                                    className="w-full rounded-lg border border-slate-300 px-3 py-2 text-sm"
                                    placeholder="เช่น ตรวจพบว่าขนาดใหญ่ขึ้น"
                                />
                            </div>
                        </div>

                        {transitionMsg && (
                            <div className={`text-sm ${transitionMsg.includes('สำเร็จ') ? 'text-emerald-600' : 'text-rose-600'}`}>
                                {transitionMsg}
                            </div>
                        )}
                ...
                <section className="rounded-2xl border border-slate-200 bg-white p-5 mb-6 space-y-3">
                    <div className="flex flex-wrap items-center justify-between gap-2">
                        <div className="flex items-center gap-2">
                            ...
                            <span className="text-xs text-slate-500">
                                {filteredSizeMoves.length.toLocaleString("th-TH")} รายการ
                            </span>
                        </div>
                    ...
                    {sizeMoveLoading && (
                        <div className="text-xs text-slate-500">กำลังโหลดประวัติ...</div>
                    )}
                    {!sizeMoveLoading && sizeMoveError && (
                        <div className="text-xs text-rose-600">โหลดไม่สำเร็จ: {sizeMoveError}</div>
                    )}
                ...
                <div className="bg-white rounded-2xl border border-slate-200 overflow-hidden mb-6">
                    <div className="px-4 py-3 border-b border-slate-100 flex items-center justify-between">
                        <span className="text-sm font-semibold text-slate-700">
                            รายการต้นไม้ในแปลง (Plot Inventory)
                        </span>
                        <span className="text-xs text-slate-500">
                            รวมทั้งสิ้น {inventoryItems.length} รายการ
                        </span>
                    </div>
                    ...
                    <div className="mx-4 mt-3 rounded-xl border border-emerald-100 bg-emerald-50/60 px-4 py-3 text-xs text-emerald-900">
                        ...
                        <p className="mt-2 text-[11px] text-emerald-800">
                            * ข้อมูลในส่วน "ผลสำรวจจำนวนต้นไม้ในแปลง" ใช้สำหรับตรวจสอบจำนวนจริงในแปลง เปรียบเทียบกับจำนวน Tag (ไม่ได้ใช้เป็นฐานขุดล้อมโดยตรง)
                        </p>
                    </div>

                    {/* แถบสรุปจำนวน Tag ตามสถานะ */}
                    ...
                    {/* Section: ส่งเข้าสต็อกพร้อมขายจากแปลงนี้ */}
                    {
                        !plotInventoryLoading && !plotInventoryError && (
                    ...
                    >
                        ...
                        {speciesError && (
                            <div className="md:col-span-6 text-xs text-rose-500">
                                โหลดรายชื่อชนิดต้นไม้ไม่สำเร็จ: {speciesError}
                            </div>
                        )}
                        {newPlantMessage && (
...

                    {
                        digupOrdersError && (
                            <p className="text-sm text-rose-500">
                                โหลดประวัติคำสั่งขุดล้อมไม่สำเร็จ: {digupOrdersError}
                            </p>
                        )
                    }

                    {
                        !digupOrdersLoading && !digupOrdersError && digupOrders.length === 0 && (
                            <p className="text-sm text-slate-400">
                                ยังไม่มีการบันทึกคำสั่งขุดล้อมในแปลงนี้
                            </p>
                        )
                    }
                                    ...
                                    <tbody>
                                        {digupOrders.map((o) => (
                                            ...
                                            >
                                                ...
                                                <td className="px-3 py-2 text-slate-600">
                                                    {o.size_label ? `${o.size_label} นิ้ว` : "-"}
                                                </td>
                ...
                <section className="bg-white rounded-xl shadow-sm border border-slate-200 p-4 mb-6">
                    <div className="flex items-center justify-between mb-1">
                        <h3 className="text-base font-semibold text-slate-800">
                            ผลสำรวจจำนวนต้นไม้ในแปลง (ตามขนาด)
                        </h3>
                        {inspectionsLoading && (
                    ...
                    <p className="mb-3 text-xs text-slate-500">
                        ใช้บันทึกผลสำรวจจำนวนต้นไม้จริงในแปลง ณ วันที่ตรวจสอบ
                        เพื่อเปรียบเทียบกับจำนวน Tag และวางแผนการผลิตในระยะยาว
                        <span className="font-medium text-slate-600">
                            ข้อมูลส่วนนี้ไม่ได้ใช้เป็นฐานในการสร้างคำสั่งขุดล้อมโดยตรง
                        </span>
                    </p>

                    {
                        inspectionsError && (
                            <div className="mb-3 text-sm text-rose-600">
                                เกิดข้อผิดพลาดในการโหลดผลสำรวจ: {inspectionsError}
                            </div>
                        )
                    }
            ...
            <section className="mt-6 mb-6">
                <h3 className="text-sm font-semibold mb-2 text-slate-700">
                    ความคลาดเคลื่อนระหว่างสต็อกระบบ vs ผลสำรวจ
                </h3>
...

                {
                    stockDiffError && (
                        <div className="text-xs text-red-600">
                            ไม่สามารถโหลดข้อมูลเปรียบเทียบได้: {stockDiffError}
                        </div>
                    )
                }

                {
                    !stockDiffLoading && !stockDiffError && stockDiffRows.length === 0 && (
                        <div className="text-xs text-gray-500">
                            ยังไม่มีข้อมูลเปรียบเทียบ (อาจยังไม่มีการปลูกหรือยังไม่เคยบันทึกผลสำรวจ)
                        </div>
                    )
                }
            ...
            {/* บล็อคแก้ไขประเภทแปลง - Overview Tab */}
            {activeTab === 'overview' && (
...

                    {
                        plotTypesError && (
                            <div className="text-xs text-red-500 mb-2">
                                โหลดรายการประเภทแปลงไม่สำเร็จ: {plotTypesError}
                            </div>
                        )
                    }
            ...
            {/* ✅ กล่องสรุปความคลาดเคลื่อนของแปลง */}
            ...
            {/* ตารางความคลาดเคลื่อน (Stock vs Inspection Discrepancy) */}
